...
/*MB *timmaybay(danhsachmaybay LA, const char *Ma){
	for (int i = 0; i < LA.soluong;i++){
		if(strcmp(LA.DSMB[i] -> sohieumb, Ma)==0) return LA.DSMB[i];
	}
	return NULL;
}*/
...
bool kiemtraxoamaybay(DSMB &LA, int vt){
	if(vt < 0) return false;// khong co so hieu nay
	for (int i = vt; i < LA.soluong;i++){
...
void DocFileMayBay(DSMB &LA){
    ifstream fileInput("DSMB.txt");
    if (!fileInput.is_open()) return;

    LA.soluong = 0;
    string tmp; 
    
    // Ð?c t?ng tru?ng m?t, phân cách b?ng d?u ph?y
    while (LA.soluong < MAX_MB && getline(fileInput, tmp, ',')) {
        if (tmp.empty()) break; // H?t file

        LA.DSMB[LA.soluong] = new MB;
        
        // 1. Ð?c S? hi?u
        strcpy(LA.DSMB[LA.soluong]->sohieumb, tmp.c_str());
        
        // 2. Ð?c Lo?i máy bay
        if (getline(fileInput, tmp, ',')) {
            strcpy(LA.DSMB[LA.soluong]->loaimb, tmp.c_str());
        }
        
        // 3. Ð?c S? ch?
        if (getline(fileInput, tmp, ',')) {
            LA.DSMB[LA.soluong]->socho = atoi(tmp.c_str());
        }
...
void GhiFileMayBay(DSMB &LA){
    ...
    if (outFile.is_open()) {
        for (int i = 0; i < LA.soluong; i++) {
            outFile << LA.DSMB[i]->sohieumb << ",";
            outFile << LA.DSMB[i]->loaimb << ",";
            outFile << LA.DSMB[i]->socho << ",";
        }
...
void NhapLieuMayBay(DSMB &LA, int &StartIdx, int NumberPerPage){
	...
	while(true){
		switch(ordinal){
			...
			case 2:{
				ConstraintForOnlyNumber(nChair,ordinal,SaveOrNot,12,999);
				if(SaveOrNot){
					LA.DSMB[LA.soluong] = new MB;
