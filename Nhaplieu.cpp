...
void ConstraintsForLetterAndSpace(string &result,int &ordinal,bool &SaveOrNot,int distance)
{
	...
	while( true )
	{
		while( _kbhit() )
		{
			...
			{
				char c = (char)signal;
				if( result.length() == 0 ) c = toupper(c);
				else if(result.length() - 1 == ' ') c = toupper(c);
				    else c = tolower(c);
				cout << c;
				result+=c;
				count++;
...
// Chuan hoa ten - chu cai dau viet hoa
...
void ConstraintForDateAndTime( int &result,int &DTordinal,bool &SaveOrNot, int LimitReach)
{
    result = 0;
	int count = 0;
	int signal ;
	while(true)
	{
		while( _kbhit() )
		{
			...
			{
                if (result > LimitReach || (DTordinal >  1 && result == 0 ))// () thu hai de kiem tra ngay thang nam khong bang 0
                    continue;
				if(count == 0) continue;
				    SaveOrNot = true; 
					return;
